...
const Role = mongoose.model('Role'); // Import the Role model
...
exports.create = async (req, res) => {
  try {
    let { email, password, role, name, surname } = req.body; // Destructure name and surname from request body

    // Check for missing fields, including name and surname
    if (!email || !password || !role || !name || !surname) {
      return res.status(400).json({
        success: false,
        result: null,
        message: "Email, password, role, name, or surname fields are missing.",
      });
    }
    ...
    const passwordHash = newAdmin.generateHash(password); // Assuming you have a method to hash passwords
    req.body.password = passwordHash; // Assign the hashed password back to req.body

    // Prepare the admin data including name and surname
    const adminData = { ...req.body, role }; 
    const result = await new Admin(adminData).save(); // Save the new admin
    ...
    return res.status(500).json({
      ...
      error: error.message || 'Unknown error', // Include the error message for clarity
    });
  }
};

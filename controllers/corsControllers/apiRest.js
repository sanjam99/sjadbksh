...
exports.create = async (Model, req, res) => {
  try {
    // Assuming you're passing an array of permission displayNames in req.body.permissions
    const permissionDisplayNames = req.body.permissions;
    
    // Fetch the corresponding ObjectIds from the Permission model based on displayName
    const permissions = await Permission.find({ displayName: { $in: permissionDisplayNames } }).select('_id');

    console.log('Requested Permissions:', permissionDisplayNames);
    console.log('Found Permissions:', permissions);
    
    // if (permissions.length !== permissionDisplayNames.length) {
    //     return res.status(400).json({
    //         success: false,
    //         result: null,
    //         message: 'Some permissions could not be found',
    //     });
    // }
    
    // Replace the permission displayNames with their corresponding ObjectIds in the request body
    req.body.permissions = permissions.map(permission => permission._id);
